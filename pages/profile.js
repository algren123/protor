...
function Profile() {
  ...
  useEffect(() => {
    ...
    async function getUser() {
      const user = await axios
        .get(
          'https://protor-backend.herokuapp.com/users?email=' +
            session.user.email
        )
        .then((res) => {
          return res.data[0];
  ...
  async function postcodeSubmit(event) {
    ...
    if (invalid || postcode === '') {
      ...
      await axios
        .post(
          'https://protor-backend.herokuapp.com/users/update/' + userId,
          userData
        )
  ...
  function deletePost(id) {
    axios
      .delete('https://protor-backend.herokuapp.com/posts/' + id)
